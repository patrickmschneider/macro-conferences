...
async function init(){
 try{
  ...
  $('#source-health').innerHTML=status.sources?.length?`<details><summary>Source check results (${status.sources.length})</summary><table><tbody>${status.sources.map(s=>`<tr><td><a target="_blank" rel="noopener" href="${link(s.url)}">${escape(s.name)}</a></td><td>${escape(s.mode||'')}</td><td>${s.state==='ok'?'Fetched':'Fetch failed'}</td><td>${s.discovery_state==='no_candidate_links'?'No usable event links':s.discovery_state==='links_found'?`${s.candidate_links} candidate links`:''}</td></tr>`).join('')}</tbody></table></details>`:'';
